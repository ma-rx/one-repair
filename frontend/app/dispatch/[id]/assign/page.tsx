...
export default function AssignPage() {
  ...
  useEffect(() => {
    Promise.all([api.getTicket(id), api.listTechs()])
      .then(([t, ts]) => { setTicket(t); setTechs(ts); })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.assignTech(id, Number(selectedTech));
      setSuccess(true);
    ...
    <DashboardShell>
      <div className="max-w-lg mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-500 text-sm mt-0.5">Select a tech to dispatch for this ticket</p>
        </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  {techs.length === 0 ? (
                    <p className="text-sm text-slate-400">No technicians found. Add users with the TECH role.</p>
                  ) : (
                    <select
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    type="submit"
                    disabled={!selectedTech || submitting || techs.length === 0}
