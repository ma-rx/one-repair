...
export default function EquipmentPage() {
  ...
  function load() {
    setLoading(true);
    api.listEquipmentModels(filterCat || undefined)
      .then(setModels)
  ...
  return (
    <DashboardShell>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        {filterCat && (
          <button onClick={() => setFilterCat("")} className="text-sm text-slate-400 hover:text-slate-600">
            Clear
          </button>
        )}
        <span className="text-slate-400 text-sm ml-auto">{models.length} model{models.length !== 1 ? "s" : ""}</span>
      </div>
